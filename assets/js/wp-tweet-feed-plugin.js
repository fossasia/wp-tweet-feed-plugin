function twp_update_twitter_auth(arg) {
	if (arg == true) {
    	jQuery("#tweet-feed-plugin-general-settings").addClass("closed");
    	jQuery(".twp_username_input").show();
    	jQuery(".twp_username_select").hide();
	}
    else {
    	jQuery("#tweet-feed-plugin-general-settings").removeClass("closed");
    	jQuery(".twp_username_input").hide();
    	jQuery(".twp_username_select").show();
    }

    jQuery("#tweet-feed-plugin-general-settings .handlediv").attr('aria-expanded', arg);
}
